    ...
    <div className="min-h-dvh">
      ...
      <div className="px-6 pt-4 pb-32">
        ...
        <div className="mt-4 px-4 py-2.5 rounded-xl bg-bg-elevated border border-border flex items-center gap-2 text-[11px] text-text-muted">
          ...
          <span>
            {t("av.reflect.skin")}: <span className="text-text">{t("av.reflect.closest")}</span> · {t("av.reflect.hair")}: <span className="text-text">{profile.typecast.hairLength || "Medium"}</span> · {t("av.reflect.eyes")}: <span className="text-text">{t("av.reflect.matched")}</span>
          </span>
        </div>

        {/* Variants */}
        <div className="mt-8 flex items-center justify-between mb-3">
          <span className="text-xs uppercase tracking-wider text-text-muted">{t("av.variations")}</span>
          <button
            onClick={regenerate}
            className="inline-flex items-center gap-1.5 text-xs text-gold hover:text-gold-light"
          >
            <RefreshCw className="w-3 h-3" /> {t("av.regenerate")}
          </button>
        </div>
      ...
      <div className="fixed bottom-0 inset-x-0 max-w-[440px] mx-auto p-4 glass border-t border-border">
        ...
        >
          {t("av.enter")} <ArrowRight className="w-4 h-4" />
        </button>
      </div>
    </div>
  );
